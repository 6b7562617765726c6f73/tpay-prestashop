...
class PaymentBasic
{
    ...
    {
        $config = $this->getBankSelectionFormConfig($config);
    ...
    public function getBankSelectionFormConfig($config, $showRegulations = true)
    {
        $config = $this->prepareConfig($config);
        $config['kanal'] = 0;
        $config['akceptuje_regulamin'] = ($showRegulations) ? 0 : 1;
        $config['merchant_id'] = $this->merchantId;
        $config['regulation_url'] = $this->regulationURL;
